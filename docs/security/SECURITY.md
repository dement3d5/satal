...
Listing risk assessment runs inside the submission transaction and stores bounded reason codes/weights only. Matched phone numbers, email addresses and URLs are not copied into signal rows or logs, signals never make an automatic enforcement decision, and only the live staff queue exposes the summaries. Row-locked assignment prevents every staff role from reviewing or deciding an unclaimed case or another staff member's claimed case. Cross-queue counts, SLA/workload metrics, recent actions and daily staff-access aggregates require `admin` or `owner` and omit internal notes, reporter identities, content, request paths, network identifiers and private account attributes.
...
The implemented ingress requires draft ownership, an exact declared size and SHA-256 digest, a ten-minute HMAC capability, a bounded uncompressed request body and JPEG/PNG/WebP magic-byte agreement. Client filenames never become object keys. Local development writes only below ignored `.data/media`; the production R2 adapter fails closed until credentials and a live end-to-end check exist. Quarantine objects are never served. The Sharp/libvips worker performs bounded real decoding, rejects animation and unsupported formats/dimensions, applies orientation and re-encodes metadata-free WebP variants. Work leases and retry state contain opaque IDs/error codes rather than file content or client filenames. Cleanup is idempotent and never deletes variants for a non-deleted asset. Public reads additionally require a `ready` asset attached to an active listing. Decoder updates, process isolation/resource limits, alert delivery, malicious corpus tests and live R2 lifecycle verification remain launch gates.

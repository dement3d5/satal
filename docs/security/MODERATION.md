...
The current workspace enforces explicit, optionally expiring PostgreSQL role grants, locks each case/report/appeal before deciding it and never returns contact, reporter identity or internal-note fields to the browser unnecessarily. Moderators and admins cannot review their own listings. The separately granted platform `owner` role has a narrow `listings:self-review` capability so a solo operator can publish its own inventory. It also has `conflicts:view` for complete operational visibility into persisted complaint/appeal queues, but the independent decision restrictions for reports, appeals, conversations and reviews remain server-enforced. Non-owner staff receive an aggregate hidden-conflict count rather than protected content, so the workspace explains an apparently empty queue without leaking it. Listing cases still require atomic self-claim before review or decision, deliberate UI confirmation, lifecycle validation and append-only decision/assignment audit. Admin/owner may release stuck work but must then explicitly claim it. Queue age is classified against a visible 18/24-hour operational target but never auto-enforces. Moderators can work queues; aggregated operations, SLA/workload metrics, recent actions and bounded staff-access projection require `admin` or `owner`. There is no public role-management endpoint and no privileged seed user. Seller appeals preserve the original rejection and append a separate decision; acceptance only reopens review, clears assignment and preserves the original risk assessment. Broader evidence access and account enforcement workflows remain later additions.

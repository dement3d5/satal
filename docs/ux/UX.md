...
Launch search starts with two explicit modes: Baku apartments and passenger cars. Switching the mode replaces the filter schema rather than carrying incompatible values across categories. Location, price and the three most useful schema-driven attributes stay visible as compact disclosure controls; less frequent attributes live in one complete category-specific disclosure. Only one disclosure may be open at a time, and clicking outside or pressing Escape closes it, preventing overlapping panels. Apartment location groups Baku districts/neighborhoods and the complete verified metro list; vehicle location deliberately excludes metro and exposes city/district choices. There is no persistent left sidebar. Exact minimum/maximum fields are retained only where a range is meaningful, such as price, area, year, mileage and floor. Boolean fields use visible three-way choices, ordinary enums use selectable tiles, and body style uses labeled vehicle silhouettes. Query submissions preserve active filters in URL state. Mobile stacks the same touch-sized controls and keeps the full-filter apply action next to the expanded fields. Other mobile journeys use a dedicated category view and sticky listing contact actions where appropriate. Do not merely shrink desktop CSS.
...
An authenticated non-seller can report an active listing from its seller card without seeing or supplying ownership identifiers. The form uses a short fixed reason list, optional bounded context and clear authenticated/rate-limited/error/success states. Seller rejection details live with the seller's own listing in the profile. One compact appeal form preserves the statement after validation and shows open, accepted and rejected outcomes plus the safe staff response. Staff work is divided into independently rendered, count-bearing queues for new listings, listing reports, message reports, review reports, appeals and operations. A new-listing task stays locked until the moderator explicitly claims it. Only the current assignee can open the dedicated review workspace with the complete description, price, localized typed characteristics and every ordered processed photo on one side and assignment, risk context and decisions on the other. Audited decisions require an explicit confirmation; reversible claim/release controls do not.
...
The localized profile route is the user's home inside Satal rather than a bare identity form. It combines a recognizable profile header, account age and public reputation link with listing-status totals, direct access to messages, saved items, notifications and staff tools, and separate personal-data and session-security cards. Email and phone remain private and verification limitations are stated next to the relevant field.

Sign-out uses Better Auth's browser client, waits for the server response and verifies that the private profile endpoint is unauthorized before leaving the page. A hard navigation then prevents stale React state from displaying the previous account. The security card exposes one unambiguous sign-out action; another account can be selected from the clean sign-in screen afterward.

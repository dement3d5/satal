...
Review ratings are constrained to 1–5 and optional text to 10–1000 trimmed characters. The API never accepts an author or subject ID: it derives the author from the session and the subject from the opposite interaction participant. A unique `(interaction_id, author_id)` index prevents duplicates and submitted reviews are immutable. An active review is public only after an active counterpart review exists or its 14-day `reveal_at` has passed. Public aggregates apply exactly the same visibility predicate so hidden or still-blind reviews cannot affect the displayed rating. See ADR 0012.

...
`GET /api/v1/moderation/cases?locale=az|ru|en&limit=30` returns a minimal localized queue DTO only to users with a live staff role. It includes bounded explainable signal codes/weights, policy version, display-only assignee state, queue age and derived SLA state, but excludes matched private content, seller email/phone, unrestricted evidence and credentials. The authorized queue access is counted in a daily bounded staff-access aggregate.

`PUT /api/v1/moderation/cases/{caseId}/assignment` atomically claims an unassigned case for the current staff actor. `DELETE` on the same resource releases the actor's assignment; admin/owner may also release another assignee's case. Effective transitions append an assignment event, while same-state retries are idempotent. `POST /api/v1/moderation/cases/{caseId}/decision` accepts a validated approve/reject union, atomically claims an unassigned case and rejects a decision assigned to another actor. Rejection requires a bounded public explanation; authorization, assignment, self-review and current case/listing state are rechecked inside the transaction.

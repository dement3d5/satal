...
`PUT /api/v1/moderation/cases/{caseId}/assignment` atomically claims an unassigned case for the current staff actor. `DELETE` on the same resource releases the actor's assignment; admin/owner may also release another assignee's case. Effective transitions append an assignment event, while same-state retries are idempotent. `POST /api/v1/moderation/cases/{caseId}/decision` accepts a validated approve/reject union and atomically records the acting reviewer. A moderator cannot override another assignee; admin/owner can and the reassignment is audited. Rejection requires a bounded public explanation; authorization, assignment, self-review and current case/listing state are rechecked inside the transaction.

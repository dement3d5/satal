...
`GET /api/v1/moderation/cases?locale=az|ru|en&limit=30` returns a localized review DTO only to users with a live staff role. It includes the submitted title, description, price, typed localized characteristics, ordered ready-photo URLs, bounded explainable signal codes/weights, policy version, display-only assignee state, queue age and derived SLA state. It excludes matched private contact content, seller email/phone, raw uploads, unrestricted evidence and credentials. The authorized queue access is counted in a daily bounded staff-access aggregate.

`GET /api/v1/moderation/media/{assetId}/variants/{kind}` serves only a `ready` processed variant attached to an open `pending_review` listing after authenticating a live staff capability. Moderators and admins are excluded from their own listing media; the platform `owner` capability may inspect its own pending listing. Responses are private and `no-store`; quarantine/original objects and active-listing public authorization are separate boundaries.

`PUT /api/v1/moderation/cases/{caseId}/assignment` atomically claims an unassigned case for the current staff actor. `DELETE` on the same resource releases the actor's assignment; admin/owner may also release another assignee's case. Effective transitions append an assignment event, while same-state retries are idempotent. `POST /api/v1/moderation/cases/{caseId}/decision` accepts a validated approve/reject union and atomically records the acting reviewer. Every staff role, including owner, must explicitly claim the case before reviewing or deciding it; an admin/owner recovering another assignee's work must release and then claim it as two audited steps. Moderators and admins cannot review their own listing. The explicit platform `owner` capability may review its own listing for solo-operation continuity, while assignment, lifecycle, confirmation and append-only audit requirements remain unchanged. Rejection requires a bounded public explanation; authorization and current case/listing state are rechecked inside the transaction.

`GET /api/v1/moderation/operations?windowDays=7|30&limit=20` is limited to live `admin` and `owner` grants. It returns current queue/assignment/SLA counts, the oldest open listing age, bounded decision aggregates, recent append-only decision/assignment actions and a bounded daily staff-access projection. Access rows contain actor display name, surface, count and last-access time only. The response omits notes, reporter identities, message/listing content, account risk data, routes, network identifiers and contact details. Responses are private and `no-store`.

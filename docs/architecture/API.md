...
`GET /api/v1/moderation/media/{assetId}/variants/{kind}` serves only a `ready` processed variant attached to an open `pending_review` listing after authenticating a live staff capability and excluding the seller. Responses are private and `no-store`; quarantine/original objects and active-listing public authorization are separate boundaries.

`PUT /api/v1/moderation/cases/{caseId}/assignment` atomically claims an unassigned case for the current staff actor. `DELETE` on the same resource releases the actor's assignment; admin/owner may also release another assignee's case. Effective transitions append an assignment event, while same-state retries are idempotent. `POST /api/v1/moderation/cases/{caseId}/decision` accepts a validated approve/reject union and atomically records the acting reviewer. Every staff role, including admin/owner, must explicitly claim the case before reviewing or deciding it; an admin/owner recovering another assignee's work must release and then claim it as two audited steps. Rejection requires a bounded public explanation; authorization, assignment, self-review and current case/listing state are rechecked inside the transaction.
